...
path = "./data/pre"
path_sim = './data/raw/MICRODADOS'
path_eleitorado = './data/raw/ELEITORADO'
path_csv = './data/pre/csv_consolidados'

# Anos pares de 2004 a 2020
anos = list(range(2021, 2023, 1))
...
def processar_ano(ano):
    ...
    # Cria pasta para CSVs do ano (mesma estrutura dos gráficos)
    pasta_csv = os.path.join(path, 'csv', str(ano))
    os.makedirs(pasta_csv, exist_ok=True)

    # Carregar dados
    arquivo_sim = os.path.join(path_sim, f'SIM-DATASUS-{ano}.csv')
    arquivo_el = os.path.join(path_eleitorado, f'eleitorado_{ano}.csv')
    ...
    mx_idade.to_csv(os.path.join(pasta_csv, f'mx_idade_{ano}.csv'), index=False, encoding='utf-8-sig')
    mx_idade_sexo.to_csv(os.path.join(pasta_csv, f'mx_idade_sexo_{ano}.csv'), index=False, encoding='utf-8-sig')
    mx_idade_uf.to_csv(os.path.join(pasta_csv, f'mx_idade_uf_{ano}.csv'), index=False, encoding='utf-8-sig')
    mx_idade_ec.to_csv(os.path.join(pasta_csv, f'mx_idade_estado_civil_{ano}.csv'), index=False, encoding='utf-8-sig')
    mx_idade_esc.to_csv(os.path.join(pasta_csv, f'mx_idade_escolaridade_{ano}.csv'), index=False, encoding='utf-8-sig')
    print(f"  CSVs do ano salvos em: {pasta_csv}")
...
if __name__ == "__main__":
    ...
    print(f"  CSVs consolidados salvos em: {path_csv}\\")
    print(f"{'='*60}")